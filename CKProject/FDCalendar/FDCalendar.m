...
- (instancetype)initWithCurrentDate:(NSDate *)date {
    if (self = [super init]) {
        ...
        [self setFrame:CGRectMake(DeviceWidth/16, 0, DeviceWidth-DeviceWidth/8, CGRectGetMaxY(self.scrollView.frame))];
        [self setupTitleBar];
...
- (void)setupTitleBar {
    ...
    titleView.backgroundColor = [UIColor colorWithRed:255.f/255.f green:99.f/255.f blue:99.f/255.f alpha:1.0f];
    ...
    UIButton *titleButton = [[UIButton alloc] initWithFrame:CGRectMake(DeviceWidth/4, 0, DeviceWidth/2, 44)];
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        UILabel *weekdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(offsetX, 50, (self.frame.size.width - 10) / count, 20)];
        weekdayLabel.textAlignment = NSTextAlignmentCenter;
        weekdayLabel.text = Weekdays[i];
        [weekdayLabel setFont:[UIFont systemFontOfSize:DeviceWidth/22.8]];
//        if (i == 0 || i == count - 1) {
//            weekdayLabel.textColor = [UIColor colorWithRed:255.f/255.f green:99.f/255.f blue:99.f/255.f alpha:1.0];
//        } else {
//        }
...
- (void)setupScrollView {
    ...
    [self.scrollView setFrame:CGRectMake(0, 75, self.frame.size.width, self.centerCalendarItem.frame.size.height)];
...
- (void)setupCalendarItems {
    ...
    itemFrame.origin.x = DeviceWidth-DeviceWidth/8;
    ...
    itemFrame.origin.x = ( DeviceWidth-DeviceWidth/8) * 2;
