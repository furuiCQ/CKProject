...
+ (UIImage *)compressImage:(UIImage *)image compressRatio:(CGFloat)ratio maxCompressRatio:(CGFloat)maxRatio
{
    ...
    int MIN_UPLOAD_RESOLUTION = 640 * 640;
    ...
    NSLog(@"before imageData length:%lu",(unsigned long)[imageData length]);

    while ([imageData length] > MAX_UPLOAD_SIZE && compression > maxCompression) {
        compression -= 0.10;
        imageData = UIImageJPEGRepresentation(image, compression);
    }
    NSLog(@"after imageData length:%lu",(unsigned long)[imageData length]);
